...
class graphics:
    ...
    def set_right_click_action(self, callee):
        ...
        def right_click_action(event):
            # maybe can add board or players
            callee(self, event.x, event.y)
    ...
    def delete_object(self, shape):
        '''removes object
        '''
        self.canvas.delete(shape)
   
    def appendImages(self, path, image_name):  #Call appendImages as many times as needed
        ''' appends image to list, can be done when loading the game
        '''

        im = Image.open(path + image_name)
        im = im.resize((90, 130))
        self.ph.append(ImageTk.PhotoImage(im))
        self.ph_name.append(image_name)

    def print_images(self, x, y, name):
        '''if you need the specific name of an image, creates corresponding image at (x,y)
        '''
        for i in range(len(self.ph)):
            ph_name = self.ph_name[i]
            if name == ph_name[:len(ph_name) - 4]:
                self.canvasImage=self.canvas.create_image(x, y, anchor='nw', image=self.ph[i])
                break
